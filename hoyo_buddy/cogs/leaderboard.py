...
class LeaderboardCog(commands.GroupCog, name=app_commands.locale_str("lb")):
    ...
    @app_commands.describe(
        ...
        calculation_id=app_commands.locale_str("Leaderboard to view", key="akasha_calculation_param_desc"),
    ...
    async def akasha_command(
        ...
        for category in categories:
            weapon = next((weapon for weapon in category.weapons if weapon.calculation_id == calculation_id), None)
        ...
        lb_embed = (
            DefaultEmbed(locale, self.bot.translator, title=category.name, description=category.details)
        ...
        view = AkashaLbPaginator(
            calculation_id, lb_embed, you, category.count, author=i.user, locale=locale, translator=self.bot.translator
        )
        await view.start(i)

    @akasha_command.autocomplete("character_id")
    async def character_id_autocomplete(self, i: Interaction, current: str) -> list[app_commands.Choice[str]]:
        locale = await get_locale(i)
    ...
    async def calculation_id_autocomplete(self, i: Interaction, current: str) -> list[app_commands.Choice[str]]:
        locale = await get_locale(i)
        ...
        choices: list[app_commands.Choice[str]] = [
            app_commands.Choice(name=f"{category.name} - {weapon.name}", value=weapon.calculation_id)
            for category in categories
    ...
    async def lb_view_command(
        self, i: Interaction, lb: str, account: app_commands.Transform[HoyoAccount, HoyoAccountTransformer]
    ) -> None:
        try:
