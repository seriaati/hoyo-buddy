...
class TypeOneModal(Modal):
    ...
    threshold = TextInput(label=LocaleStr(key="notif_modal.threshold.label"), is_digit=True, min_value=0, max_value=240)
    ...
    def __init__(
        self, notes_notify: NotesNotify | None, *, title: LocaleStr, threshold_max_value: int, min_notify_interval: int
    ) -> None:
        ...
        if notes_notify is not None:
            ...
            self.threshold.default = str(notes_notify.threshold) if notes_notify.threshold is not None else ""
