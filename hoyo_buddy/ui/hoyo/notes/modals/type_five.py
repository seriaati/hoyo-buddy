...
class TypeFiveModal(Modal):
    ...
    notify_interval = TextInput(
        label=LocaleStr(key="notif_modal.notify_interval.label"), is_digit=True, max_value=DB_SMALLINT_MAX
    )
    ...
    hours_before = TextInput(
        label=LocaleStr(key="notif_modal.hours_before.label"), is_digit=True, min_value=1, max_value=DB_SMALLINT_MAX
    )
