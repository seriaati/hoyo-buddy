...
class BountyCommissionReminder(Button[NotesView]):
    def __init__(self, *, row: int) -> None:
        super().__init__(label=LocaleStr(key="bounty_commission", mi18n_game=Game.ZZZ), row=row)

    async def callback(self, i: Interaction) -> None:
        notify = await NotesNotify.get_or_none(
            account=self.view.account, type=NotesNotifyType.ZZZ_BOUNTY
        )
        ...
        embed = await self.view.process_type_four_modal(
            modal=modal, notify=notify, notify_type=NotesNotifyType.ZZZ_BOUNTY, check_interval=30
        )
