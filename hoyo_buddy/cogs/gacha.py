...
class Gacha(
    ...
    async def import_(
        ...
        account_ = account or await self.bot.get_account(i.user.id, (Game.GENSHIN, Game.ZZZ, Game.STARRAIL))
        await GachaCommand().run_import(i, account_)

    @app_commands.command(
        name=app_commands.locale_str("view"),
        description=app_commands.locale_str("View imported gacha logs", key="gacha_view_command_description"),
    )
    @app_commands.rename(account=app_commands.locale_str("account", key="account_autocomplete_param_name"))
    @app_commands.describe(
        account=app_commands.locale_str(
    ...
    @app_commands.command(
        ...
        description=app_commands.locale_str("Manage imported gacha logs", key="gacha_manage_command_description"),
    )
    @app_commands.rename(account=app_commands.locale_str("account", key="account_autocomplete_param_name"))
    @app_commands.describe(
        account=app_commands.locale_str(
    ...
    @app_commands.describe(
        source=app_commands.locale_str("Source of the gacha history", key="source_param_description"),
    ...
    @app_commands.choices(
        source=[app_commands.Choice(name=source.value, value=source.value) for source in GachaImportSource]
    )
    async def upload(
        self,
        i: Interaction,
